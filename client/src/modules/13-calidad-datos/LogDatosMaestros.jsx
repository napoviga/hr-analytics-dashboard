...
export default function LogDatosMaestros() {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function fetchData() {
      const { data: rawData, error } = await supabase.schema('business').from('mv_monthly_kpis_bynapo').select('*').limit(200);
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500">Cargando métricas de Calidad de Datos...</div>;
  }
  ...
  return (
    <DashboardLayout 
      ...
      description="Métricas de completitud."
      kpiCards={
        <>
          <KpiCard title="Total Registros" value={totalRecords} subtitle="Analizados en mv_monthly_kpis_bynapo" />
