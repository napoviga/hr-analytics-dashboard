...
export default function TiempoProductividad() {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function fetchData() {
      const { data: rawData, error } = await supabase.schema('business').from('mv_onboarding_status').select('*').limit(200);
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500">Cargando métricas de Tiempo a Productividad...</div>;
  }
  ...
  return (
    <DashboardLayout 
      ...
      description="Curvas de aprendizaje."
      kpiCards={
        <>
          <KpiCard title="Total Registros" value={totalRecords} subtitle="Analizados en mv_onboarding_status" />
