...
export default function SaludOcupacional() {
  ...
  useEffect(() => {
    async function fetchData() {
      const { data: rawData, error } = await supabase.schema('business').from('mv_sst_incidents').select('*').limit(200);
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500">Cargando métricas de Salud Ocupacional...</div>;
  }
  ...
  return (
    <DashboardLayout 
      ...
      description="Incidentes SST."
      kpiCards={
        <>
          <KpiCard title="Total Registros" value={totalRecords} subtitle="Analizados en mv_sst_incidents" />
