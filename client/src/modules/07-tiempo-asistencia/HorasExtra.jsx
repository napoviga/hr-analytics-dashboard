...
export default function HorasExtra() {
  ...
  useEffect(() => {
    async function fetchData() {
      const { data: rawData, error } = await supabase.schema('business').from('mv_overtime_summary').select('*').limit(200);
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500">Cargando métricas de Horas Extra...</div>;
  }
  ...
  return (
    <DashboardLayout 
      title="Horas Extra"
      description="Costo y horas extra."
      kpiCards={
        <>
          <KpiCard title="Total Registros" value={totalRecords} subtitle="Analizados en mv_overtime_summary" />
