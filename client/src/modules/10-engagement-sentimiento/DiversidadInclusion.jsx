...
export default function DiversidadInclusion() {
  ...
  useEffect(() => {
    async function fetchData() {
      const { data: rawData, error } = await supabase.schema('business').from('mv_enps_trend').select('*').limit(200);
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500">Cargando métricas de Diversidad e Inclusión...</div>;
  }
  ...
  return (
    <DashboardLayout 
      ...
      description="Métricas D&I."
      kpiCards={
        <>
          <KpiCard title="Total Registros" value={totalRecords} subtitle="Analizados en mv_enps_trend" />
