...
export default function ImpactoFinanciero() {
  ...
  useEffect(() => {
    async function fetchData() {
      const { data: rawData, error } = await supabase.schema('business').from('mv_turnover_cost').select('*').limit(200);
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500">Cargando métricas de Impacto Financiero...</div>;
  }
  ...
  return (
    <DashboardLayout 
      ...
      description="Costos de rotación."
      kpiCards={
        <>
          <KpiCard title="Total Registros" value={totalRecords} subtitle="Analizados en mv_turnover_cost" />
