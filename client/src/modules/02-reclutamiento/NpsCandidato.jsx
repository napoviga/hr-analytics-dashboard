...
export default function NpsCandidato() {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function fetchData() {
      const { data: rawData, error } = await supabase.schema('business').from('mv_recruitment_funnel').select('*').limit(200);
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500">Cargando métricas de NPS del Candidato...</div>;
  }
  ...
  return (
    <DashboardLayout 
      ...
      description="Satisfacción de candidatos."
      kpiCards={
        <>
          <KpiCard title="Total Registros" value={totalRecords} subtitle="Analizados en mv_recruitment_funnel" />
