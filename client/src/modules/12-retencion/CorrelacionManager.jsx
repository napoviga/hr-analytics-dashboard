...
export default function CorrelacionManager() {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function fetchData() {
      const { data: rawData, error } = await supabase.schema('business').from('mv_manager_turnover').select('*').limit(200);
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500">Cargando métricas de Correlación Manager...</div>;
  }
  ...
  return (
    <DashboardLayout 
      title="Correlación Manager"
      description="Rotación por manager."
      kpiCards={
        <>
          <KpiCard title="Total Registros" value={totalRecords} subtitle="Analizados en mv_manager_turnover" />
