...
export default function ComportamientoGrupos() {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function fetchData() {
      const { data: rawData, error } = await supabase.schema('business').from('mv_lifecycle_cohorts').select('*').limit(200);
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500">Cargando métricas de Cohortes por Antigüedad...</div>;
  }
  ...
  return (
    <DashboardLayout 
      title="Cohortes por Antigüedad"
      description="Comportamiento por grupos."
      kpiCards={
        <>
          <KpiCard title="Total Registros" value={totalRecords} subtitle="Analizados en mv_lifecycle_cohorts" />
