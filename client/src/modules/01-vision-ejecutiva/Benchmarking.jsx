...
export default function Benchmarking() {
  ...
  useEffect(() => {
    async function fetchData() {
      const { data: rawData, error } = await supabase.schema('business').from('mv_salary_bands').select('*').limit(200);
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500">Cargando métricas de Benchmarking...</div>;
  }
  ...
  return (
    <DashboardLayout 
      title="Benchmarking"
      description="Comparativas ejecutivas."
      kpiCards={
        <>
          <KpiCard title="Total Registros" value={totalRecords} subtitle="Analizados en mv_salary_bands" />
