...
export default function AlertasAnomalias() {
  ...
  useEffect(() => {
    async function fetchData() {
      const { data: rawData, error } = await supabase.schema('business').from('mv_alerts_anomalies').select('*').limit(200);
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500">Cargando métricas de Alertas y Anomalías...</div>;
  }
  ...
  return (
    <DashboardLayout 
      ...
      description="Z-Score outliers."
      kpiCards={
        <>
          <KpiCard title="Total Registros" value={totalRecords} subtitle="Analizados en mv_alerts_anomalies" />
