...
export default function RoiCapacitacion() {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function fetchData() {
      const { data: rawData, error } = await supabase.schema('business').from('mv_training_roi').select('*').limit(200);
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500">Cargando métricas de ROI Capacitación...</div>;
  }
  ...
  return (
    <DashboardLayout 
      title="ROI Capacitación"
      description="Retorno de inversión."
      kpiCards={
        <>
          <KpiCard title="Total Registros" value={totalRecords} subtitle="Analizados en mv_training_roi" />
