...
export default function MapaSucesion() {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function fetchData() {
      const { data: rawData, error } = await supabase.schema('business').from('mv_nine_box').select('*').limit(200);
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500">Cargando métricas de Mapa Sucesión...</div>;
  }
  ...
  return (
    <DashboardLayout 
      title="Mapa Sucesión"
      description="Planes de sucesión."
      kpiCards={
        <>
          <KpiCard title="Total Registros" value={totalRecords} subtitle="Analizados en mv_nine_box" />
